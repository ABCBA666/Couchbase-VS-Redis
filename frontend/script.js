...
function getFormData() {
    return {
        dataSize: parseInt(document.getElementById('dataSize').value) || 1024,
        concurrency: parseInt(document.getElementById('concurrency').value) || 10,
        totalOperations: parseInt(document.getElementById('totalOperations').value) || 1000
    };
}
...
function showLoading(elementId) {
    ...
    element.innerHTML = '<div class="loading"></div> 测试进行中...';
}

// 显示错误信息
function showError(elementId, error) {
    const element = document.getElementById(elementId);
    element.innerHTML = `<div class="alert alert-danger">测试失败: ${error}</div>`;
}
